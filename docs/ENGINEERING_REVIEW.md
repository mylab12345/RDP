...
| REL-04 | Medium | `SessionStore.get()` exposes mutable internal models, and most mutating methods update memory before persistence. A failed save can therefore leave process state ahead of disk. | **Partially resolved:** `update()`, `delete()` and `upsert()` are now transactional (2026-09-22); remaining mutating methods and copy-returning read APIs still open. |
...
| TEST-01 | Medium | CI has no Windows job, native X11/Wayland smoke test, FreeRDP compatibility matrix, or coverage trend. Linux offscreen tests cannot validate native embedding/input end to end. | **Partially resolved:** Windows import/CLI/core smoke job added (2026-09-22, first pass); native X11/Wayland smoke, FreeRDP matrix, and coverage trend remain open. |
| REL-05 | Medium | Full offscreen suite can complete its results and then hang in Qt/worker teardown for tens of minutes (observed 2026-09-22, `pytest tests`); a CI gate would time out. | **Open:** bisect the integration-tail fixture teardown; add a CI-level timeout as a guard. |
...
| OBS-01 | Low/Medium | Numerous broad catches are justified at process/UI boundaries, but several silently discard optional-backend errors. Diagnosis depends on reproducing with debug logging. | **Partially resolved:** rate-limited debug records applied across SSH worker/session teardown paths; remaining modules to be covered as they are touched. |
