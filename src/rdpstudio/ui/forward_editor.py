"""Editor widgets for port-forward definitions."""
...
class ForwardDialog(QDialog):
    def __init__(self, fwd: Forward | None = None, parent=None) -> None:
        ...
        self.kind = QComboBox()
        self.kind.addItem("Local  (local port → remote destination)", "local")
        self.kind.addItem("Remote  (remote port → local destination)", "remote")
        self.kind.addItem("Dynamic (local SOCKS5 proxy)", "dynamic")
        ki = self.kind.findData(fwd.kind)
        self.kind.setCurrentIndex(ki if ki >= 0 else 0)
        form.addRow("Type", self.kind)

        self.name = QLineEdit(fwd.name)
        form.addRow("Label (optional)", self.name)
...
class ForwardListEditor(QWidget):
    """Table of forwards with add/edit/remove (used in session editor)."""
    ...
    def _append(self, f: Forward) -> None:
        item = QTreeWidgetItem(
            ["✓" if f.enabled else "·", f"{f.listen_host}:{f.listen_port}", f.dest_label(), f.name]
        )
